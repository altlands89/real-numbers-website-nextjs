...
export function QuestionsVisualEditorClient({ initialData, initialFaqItems, mediaLibrary, pageUrl }: Props) {
  ...
  const canUndo = canUndoData || canUndoFaq || canUndoOverrides || canUndoDesktopWidth || canUndoMobileWidth;
  const canRedo = canRedoData || canRedoFaq || canRedoOverrides || canRedoDesktopWidth || canRedoMobileWidth;
  const handleUndo = () => history.undo(historySlices);
  const handleRedo = () => history.redo(historySlices);

  const overallDirty = dirty || faqDirty || overridesDirty || desktopWidthDirty || mobileWidthDirty;
